...
function toText(report) {
  ...
  lines.push(
    ...
    `Arrival: ${formatTime12(report.arrival) || '—'}`,
    `Departure: ${formatTime12(report.departure) || '—'}`,
...
<style>
  ...
  <div class="foot">
    ...
    <div><strong>Arrival Time</strong><br/>${esc(formatTime12(report.arrival))}</div>
    <div><strong>Departure Time</strong><br/>${esc(formatTime12(report.departure))}</div>
...
/** Convert "HH:MM" (24h) or already-12h strings to "h:mm AM/PM" for exports */
function formatTime12(raw) {
  const s = String(raw || '').trim();
  if (!s) return '';
  // already has AM/PM
  if (/[ap]\.?m\.?$/i.test(s)) {
    return s.replace(/\s+/g, ' ').toUpperCase().replace(/\.+/g, '');
  }
...
async function fillCompanyPdf(report) {
  ...
  setText(form, '106', formatTime12(report.arrival));
  setText(form, '107', formatTime12(report.departure));
...
export function mountDaily(el, preload = null) {
  ...
  el.innerHTML = `
    ...
    <div class="card">
      ...
      <p class="muted">Hours auto-fill from arrival → departure. Downloads use <strong>12-hour</strong> time (e.g. 2:30 PM).</p>
    </div>
